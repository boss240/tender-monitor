...
if __name__ == "__main__":
    print("🔍 Запуск сканування тендерів...")
    
    # 1. Отримуємо тендери з Prozorro (та інших джерел, якщо підключені)
    tenders = get_new_tenders() # Або get_tenders() — залежно від того, як названа ваша функція
    
    # 2. Зберігаємо результати для дашборду (завжди)
    with open("results.json", "w", encoding="utf-8") as f:
        json.dump(tenders, f, ensure_ascii=False, indent=2, default=str)
        
    # Якщо використовуєте функцію оновлення HTML всередині файлу
    try:
        update_dashboard(tenders)
    except NameError:
        pass # Якщо update_dashboard не використовується, просто ігноруємо
        
    # 3. Відбираємо тільки нові тендери для відправки
    new_only = [t for t in tenders if t.get("is_new")]
    
    # 4. Перевірка: локальний чи автоматичний запуск
    if sys.stdin.isatty():
        # --- ЛОКАЛЬНИЙ ЗАПУСК В КОНСОЛІ ---
        print(f"\n=====================================")
        print(f"📊 СКАНУВАННЯ ЗАВЕРШЕНО")
        print(f"✅ Всього в базі (в results.json): {len(tenders)}")
        print(f"🔔 НОВИХ ДЛЯ СПОВІЩЕННЯ В TELEGRAM: {len(new_only)}")
        print(f"=====================================\n")
        
        if len(new_only) > 0:
            print("Оберіть дію:")
            print(" [1] Відправити в Telegram зараз")
            print(" [2] Не відправляти (тільки оновити дашборд)")
            print(" [3] Активувати нові платформи (UNGM, TED, DREAM)")
            
            choice = input("\nВаш вибір (1/2/3): ").strip()
            
            if choice == '1':
                print("🚀 Надсилаю сповіщення...")
                asyncio.run(send_telegram(new_only))
                print("✅ Відправлено!")
            elif choice == '3':
                print("\n⚙️ Щоб активувати нові платформи, переконайтесь що:")
                print(" 1. Додані UNGM_API_KEY та TED_API_KEY у GitHub Secrets.")
                print(" 2. У scanner.py знято коментарі з виклику цих джерел.")
            else:
                print("❌ Відправку в Telegram скасовано (дашборд успішно оновлено).")
        else:
            print("☕ Нових тендерів немає.")
            
    else:
        # --- АВТОМАТИЧНИЙ ЗАПУСК (GitHub Actions) ---
        if new_only:
            asyncio.run(send_telegram(new_only))
            print(f"✅ Автоматично надіслано в Telegram: {len(new_only)} нових тендерів")
        else:
            print("✅ Нових тендерів немає, скрипт завершив роботу.")
